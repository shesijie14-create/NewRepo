...
void user_io_config(void)
{
	   // uint8_t bklight = MACRO_PIN(PIN_LCD_BKLIGHT);
		struct pwm_device *p_pwm = NULL;
	
#if 0
		if (bklight != 255)
		{
			gpio_iomap_output(bklight, GPIO_IOMAP_OUTPUT);
			gpio_set_val(bklight, 1);
		}
#endif
		gpio_set_mode(PA_9, GPIO_PULL_UP, GPIO_PULL_LEVEL_NONE);
		gpio_set_mode(PA_10, GPIO_PULL_UP, GPIO_PULL_LEVEL_NONE);
	
		p_pwm = (struct pwm_device*)dev_get(HG_PWM0_DEVID);
		if (!p_pwm) 
		{
			return RET_ERR;
		}
		//pwm_init(p_pwm, PWM_CHANNEL_1, 10000, 10000/2);
		pwm_init(p_pwm, PWM_CHANNEL_1, 10000, 8000);
		pwm_start(p_pwm, PWM_CHANNEL_1);  // backlight open


#if 0

    uint8_t pwr_on = MACRO_PIN(PIN_PWR_ON);

    if (pwr_on != 255)
    {
        gpio_iomap_output(pwr_on, GPIO_IOMAP_OUTPUT);
        gpio_set_val(pwr_on, 1);
    }

    uint8_t spk_mute = MACRO_PIN(PIN_SPK_MUTE);

    if (spk_mute != 255)
    {
        gpio_iomap_output(spk_mute, GPIO_IOMAP_OUTPUT);
        gpio_set_val(spk_mute, 0);
    }
