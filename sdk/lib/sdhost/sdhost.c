...
extern void sd_plugin_handle(void);
extern void sd_plugout_handle(void);
...
int32 sdh_loop(struct os_work *work)
{
    ...
    static uint8 old_state = SD_IDLE;
	...
	if((old_state==SD_IDLE)&&(host->sd_opt==SD_OFF))
	{
		sd_plugout_handle();
	}
	else if((old_state==SD_M_R)&&(host->sd_opt==SD_IDLE))
	{
		sd_plugin_handle();
	}
	old_state = host->sd_opt;
